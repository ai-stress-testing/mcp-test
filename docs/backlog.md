...
| MT-31 | Build the owner analytics dashboard UI | `frontend/react-dev` | sprint-7-26-19-26 | done (`pipeline/web/admin/`, 0 axe ×6; login/pricing/funnel/heatmap; aggregate-only, honest below-sample states); orchestrator wired `/admin` serving + verified | [brief](sprint-7-26-19-26/product-brief.md) |
| MT-32 | Serve `web/` static assets + `/admin` from the app | `backend/backend-dev` | sprint-7-26-19-26 | done (orchestrator: StaticFiles mount + /admin route; all assets 200, API un-shadowed, verified) | [brief](sprint-7-26-19-26/product-brief.md) |
